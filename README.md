...
**4. Use Case Diagrams:**<br>
  ...
  - Utente manager:
    <img src="http://yuml.me/diagram/scruffy/usecase/[Utente manager]-(Inserimento eventi),(Inserimento eventi)>(Inserimento dati evento),(Inserimento dati evento)>(Nome evento),(Inserimento dati evento)>(Descrizione evento),(Inserimento dati evento)>(Nome artista),(Inserimento dati evento)>(Luogo),(Luogo)<(Seleziona file per visualizzatore VR),(Inserimento dati evento)>(Data),(Inserimento dati evento)>(Numero posti),(Inserimento dati evento)>(Prezzo per biglietto),[Utente manager]-(Visualizza profilo utente),[Utente manager]-(Logout),[Utente manager]-(Rispondere alle recensioni),[Utente manager]-(Segnalare una recensione)">
